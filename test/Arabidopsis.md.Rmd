...
```{r}
# MITETracker
ref_MT_compare <- read.delim("Arabidopsis_ref_MITE_MITETracker_overlap.gff3",header=FALSE)
names(ref_MT_compare)[1:9] <- c("Chrom","Source","Type","Start","End","Score","Strand","Phase","Attributes")
ref_MT_compare_format <- dplyr::select(ref_MT_compare, c(1,4,5))
#MTplot <- chromPlot(annot1 = ref_MT_compare_format)

# MITEFinder
ref_MF_compare<- read.delim("Arabidopsis_ref_MITE_MITEFinder_overlap.gff3", header = FALSE)
names(ref_MF_compare)[1:9] <- c("Chrom","Source","Type","Start","End","Score","Strand","Phase","Attributes")
ref_MF_compare_format <- dplyr::select(ref_MF_compare, c(1,4,5))
