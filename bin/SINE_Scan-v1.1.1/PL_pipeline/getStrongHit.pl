...
system "/usr/local/Cellar/blast/2.10.0/bin/blastn -task blastn -db $genome -query $te -max_target_seqs 100000 -evalue 1e-5 -dust no -out $prefix.te2genome.bls -num_threads $cpu -outfmt 6";
...
system "/usr/local/Cellar/bedtools/2.29.2/bin/bedtools merge -i $prefix.sortGFF >$prefix.mergebed";
print "Get sequences of these merge regions\n";
system "/usr/local/Cellar/bedtools/2.29.2/bin/bedtools getfasta -fi $genome -bed $prefix.mergebed -fo $prefix.mergebed.fasta";
print "Blast these merged region to TE base. Only the first best hit is kept\n";
system "/usr/local/Cellar/blast/2.10.0/bin/blastn -task blastn -db $te -query $prefix.mergebed.fasta -max_target_seqs 1 -evalue 1e-5 -out $prefix.mergebed.TEBase.bls -num_threads $cpu -outfmt 6";
